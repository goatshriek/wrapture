...
module Wrapture
  ...
  class PythonWrapper
    ...
    # Generates the pyproject.toml and and other supporting files for the
    ...
    def self.write_spec_pyproject_files(spec, **kwargs)
      wrapper = new(spec)
      wrapper.write_pyproject_files(**kwargs)
    end
    ...
    # Generates pyproject.toml and other supporting files for this
    # instance's spec. These can be used to package the files generated by
    # +write_source_files+.
    def write_pyproject_files(dir: Dir.pwd)
      ...
      File.open(File.join(dir, 'pyproject.toml'), 'w') do |file|
        file.puts <<~PYPROJECTTEXT
          [project]
          name = "#{@spec.name}"
          version = "1.0" # TODO: create a scope version number
          description= "#{@spec.doc.text}"

          [tool.setuptools]
          ext-modules = [
            {name="#{@spec.name}", language="c", sources=["#{@spec.name}.c"], libraries=[#{libraries}]}
          ]
